-- =====================================================
-- CRIAÇÃO DAS TABELAS PARA O SISTEMA DE QUESTÕES
...
-- Tabela questoes (estrutura simplificada)
...
-- Políticas para questões (leitura pública)
...
CREATE POLICY "Alunos podem ver apenas suas próprias respostas" ON respostas_usuarios FOR SELECT USING (auth.uid()::integer = usuario_id);
CREATE POLICY "Alunos podem inserir suas próprias respostas" ON respostas_usuarios FOR INSERT WITH CHECK (auth.uid()::integer = usuario_id);
CREATE POLICY "Alunos podem atualizar apenas suas próprias respostas" ON respostas_usuarios FOR UPDATE USING (auth.uid()::integer = usuario_id);

-- Políticas para comentários
CREATE POLICY "Comentários são visíveis para todos" ON comentarios_alunos FOR SELECT USING (true);
CREATE POLICY "Alunos podem inserir seus próprios comentários" ON comentarios_alunos FOR INSERT WITH CHECK (auth.uid()::integer = usuario_id);
CREATE POLICY "Alunos podem atualizar apenas seus próprios comentários" ON comentarios_alunos FOR UPDATE USING (auth.uid()::integer = usuario_id);
CREATE POLICY "Alunos podem deletar apenas seus próprios comentários" ON comentarios_alunos FOR DELETE USING (auth.uid()::integer = usuario_id);
...
CREATE TRIGGER update_respostas_usuarios_updated_at BEFORE UPDATE ON respostas_usuarios FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
CREATE TRIGGER update_comentarios_alunos_updated_at BEFORE UPDATE ON comentarios_alunos FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
