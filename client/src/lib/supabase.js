...
export const supabaseClient = createClient(supabaseUrl, supabaseAnonKey, supabaseConfig)
...
export const app = {
  ...
  // Responder questão
  responderQuestao: async (questaoId, resposta, correta) => {
  ...
  adicionarComentario: async (questaoId, texto) => {
    const { data: { user } } = await supabase.auth.getUser()
  ...
  adicionarQuestaoCaderno: async (cadernoId, questaoId) => {
    const { data, error } = await supabase
